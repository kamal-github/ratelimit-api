...
func (s *BucketStore) Load(_ context.Context, key string) (ratelimit.BucketState, bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()
...
// the bucket's LastRefill timestamp. The CAS check is load-bearing here, not
// a formality: the mutex only makes each individual Load or Save call
// atomic — the algorithm calls them as two separate operations, so another
// goroutine can still run its own Load-decide-Save in between this call's
// Load and this call's Save. Without the CAS check, that interleaving is a
// silent lost update.
func (s *BucketStore) Save(_ context.Context, key string, newState ratelimit.BucketState, expectedLastRefill int64, ttl time.Duration) (bool, error) {
	s.mu.Lock()
	defer s.mu.Unlock()

	currentLastRefill := int64(0)
	if existing, ok := s.data[key]; ok && time.Now().Before(existing.expiresAt) {
		currentLastRefill = existing.state.LastRefill
	}

	if currentLastRefill != expectedLastRefill {
		return false, nil
	}

	s.data[key] = bucketEntry{state: newState, expiresAt: time.Now().Add(ttl)}
	return true, nil
}
